...
| UI/UX reskin — Axie theme (new, this session) | The interface still read as the inherited wuxia prototype (dark ink-wash panels, gold-scroll borders, a serif display font) even though the game itself is already reskinned to Axie. Replaced the whole visual language: new blue/indigo CSS design-token palette (`style.css`, panels/buttons/bars/HUD), the display font swapped from `Playfair Display` (serif, wuxia scroll feel) to `Baloo 2` (rounded, playful), buttons rebuilt as solid candy-gradient pills with a pressed 3D shadow, and every leftover gold/tan/dark-ink hex color still hardcoded in `game.js`'s inline styles (float text, panel footnotes, dialogue, lore) remapped to the new palette. Added a `.choice-card` style so "pick 1 of N" option buttons (skill-evolution paths, tower-draft cards) read as distinct from single confirm/action buttons, which are now solid-filled. Legitimate reward/success gold (`#ffd76a`, `#e8b04a`, class/rarity colors) intentionally left untouched. |
...
| Art — NPCs (15/15), trees/rocks (8/11) | All 8 region trees are sourced. All 15 NPC portraits now use real sourced Axie/Chimera art — the last 12 (blacksmith→forge, herbalist, tomb keeper, etc.) came from `axieinfinity/unity-axie-gtk2d`'s "Chimera" and "starter" character sheets on GitHub, cropped and hand-matched to each NPC's role. Only the 3 decoration rocks have no source match — see `docs/ASSET_SOURCING.md` |
| Audio — BGM (15/15 map slots), SFX (12/12 names + 27 per-class), VFX (ultimate-cast overlay, 9/9 classes) | All previously-silent BGM slots (9 regions/dungeons/the romance scene) and missing SFX names (8 of 12) now play real audio, sourced from `axieinfinity/axie-origins-asset-kit`'s `PvE/Music`, `Audio`, and `web-vfx` folders on GitHub. Each of the 9 classes also now has its own basic-attack/skill-cast/ultimate sound (was one generic sound for all). The ultimate ability additionally gets a class-colored additive sprite-VFX burst layered on the existing procedural effect. See `docs/ASSET_SOURCING.md` (tenth source) |
| Story / quests / NPC dialogue | Done — rewritten from the wuxia prototype into a Lunacia/Sigils narrative (intro, main + side quest chains, NPC dialogue, boss lore). See `docs/LORE_BIBLE.md` |
