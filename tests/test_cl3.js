...
(async () => {
  ...
  const packInfo = await page.evaluate(() => MAPS.corran.packs.map((p,i) => ({i, mob:p.mob, bossKind: !!(MOBS[p.mob]&&MOBS[p.mob].bossKind)})));
  console.log('packs:', JSON.stringify(packInfo));
  // pick first non-boss pack
  const idx = packInfo.find(p => !p.bossKind)?.i ?? 0;
  await page.evaluate((idx) => { enterStage('corran', idx); }, idx);
