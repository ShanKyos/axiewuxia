...
(async () => {
  ...
  const r1 = await p.evaluate((mocs) => {
    ...
    for (const lv of mocs){
      ...
      player.chimera = { eq:null, co:{}, ve:{gk:0,cx:0}, pity5:0, pity4:0, bd:false, pity5s:0, pity4s:0, nguyet:0, tinh:0, su:[], tanthu:20 }; player.thanbinh = { tier:1 }; player.resetCount = 0;
      // ⚠ Cốt phải tháo RIÊNG. Trước đợt gỡ Ragoon nó nằm TRONG `player.chimera`, nên dòng trên
      // tháo luôn nó; nay nó ở `player.cot` và lọt thẳng vào phép đo "nhân vật KHÔNG trang bị".
      // Bốn ô Cổ +12 cộng ~+19% công / +21% máu, đủ để mục này đỏ (đo được 134% / 131%) mà trông
      // y như một đợt lạm phát chỉ số. Danh sách tháo ở đây là một DANH SÁCH TAY: dời một nguồn
      // chỉ số đi đâu là phải sửa nó theo, không có gì tự nhắc.
      player.cot = { sung:null, vuot:null, vay:null, duoi:null }; player.cotKho = [];
      player.cotBo = null; player.cotNhanhT = 0; player.skillDmgPct = 0;
